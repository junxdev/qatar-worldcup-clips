...
class Video {
  ...
  constructor(id: string, title: string, produceDateTime: number) {
    this.id = id;
    this.title = title;
    this.produceDateTime =new Date(new Date(produceDateTime).getTime() - threeHour);
  }
...
const listUrl =
  "/naver/video/lists/total?upperCategoryId=event&categoryId=qatar2022&page=1&pageSize=1000&sort=date&themeType=type&themeCode=2&fields=worldcup";
    ...
    (async () => {
      ...
      setVideos(
        x.result.videos
          .filter(
            (v: any) =>
              !v.title.startsWith("[SBS") &&
              !v.title.startsWith("[MBC") &&
              v.title.endsWith("하이라이트")
          )
          .map(
  ...
  return (
    <div className={styles.container}>
      <Head>
        ...
        <meta name="description" content="Watch 2022 Qatar World Cup clips without any spoiler" />
